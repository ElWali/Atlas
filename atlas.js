...
const biomesLayer = L.tileLayer.wms(
  ...
  { layers: "MODIS_Terra_Land_Surface_Classification_Daily", format: "image/png", transparent: true, opacity: 0.3 }
);

const oceanCurrents = L.tileLayer.wms(
  "https://gibs.earthdata.nasa.gov/wms/epsg3857/best/wmts.cgi?",
  { layers: "OSCAR_Sea_Surface_Currents", format: "image/png", transparent: true, opacity: 0.5 }
);

const starsLayer = L.tileLayer(
  "https://tiles.stamen.com/toner-background/{z}/{x}/{y}.png",
  { opacity: 0.6, attribution: "Stars data © Stellarium/NASA" }
);
...
info.update = function (latlng) { this._div.innerHTML = latlng ? `Lat: ${latlng.lat.toFixed(2)}, Lng: ${latlng.lng.toFixed(2)}` : 'Move finger: coords'; };
info.addTo(map);
...
.on('markgeocode', e => {
  ...
  searchMarker = L.marker(e.geocode.center).addTo(map).bindPopup(`<strong>${e.geocode.name}</strong>`).openPopup();
  if (e.geocode.bbox) map.flyToBounds(e.geocode.bbox,{duration:2});
  else map.flyTo(e.geocode.center,10,{duration:2});
}).addTo(map);
...
locateControl.onAdd = function () {
  ...
  btn.innerHTML = '📍'; btn.title="Go to my location";
  btn.style.fontSize='22px'; btn.style.width='44px'; btn.style.height='44px';
  L.DomEvent.on(btn,'click',()=>map.locate({setView:true,maxZoom:14,enableHighAccuracy:true}));
  return btn;
};
...
compassControl.onAdd = function () {
  const btn = L.DomUtil.create('button','leaflet-bar');
  btn.innerHTML='🧭'; btn.title="Toggle compass"; btn.style.fontSize='22px'; btn.style.width='44px'; btn.style.height='44px';
  L.DomEvent.on(btn,'click',()=>{compassEnabled=!compassEnabled; if(!compassEnabled) setMapRotation(0);});
  return btn;
};
...
compassOverlay.onAdd=function(){
  ...
  div.style.width="50px";div.style.height="50px";div.style.border="2px solid black";div.style.borderRadius="50%";div.style.background="rgba(255,255,255,0.7)";
  div.style.display="flex";div.style.alignItems="center";div.style.justifyContent="center";
  compassArrow=L.DomUtil.create('div','',div);
  compassArrow.innerHTML="N";
  compassArrow.style.fontSize="18px";compassArrow.style.fontWeight="bold";compassArrow.style.color="red";
  return div;
};
